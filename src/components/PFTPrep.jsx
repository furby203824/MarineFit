...
import { calculatePFTScore, calculateCFTScore, ageGroups } from '../utils/pftScoring';
import {
  Calculator,
  Activity,
  Trophy,
...
const PFTPrep = () => {
  ...
  const downloadWeekPDF = async (weekNum) => {
    ...
    try {
      ...
      const images = await Promise.all(workouts.map((w) => loadImage(w.imgSrc).catch(() => null)));
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-20">
        ...
        <div className="flex gap-2 mt-6 bg-black/20 p-1 rounded-lg w-fit">
          <button
            ...
            className={`px-4 py-2 rounded-md text-sm font-semibold transition-all ${
              testType === 'pft' ? 'bg-white text-marine-red shadow-sm' : 'text-red-100 hover:bg-white/10'
            }`}
          >
            PFT
          </button>
          <button
            ...
            className={`px-4 py-2 rounded-md text-sm font-semibold transition-all ${
              testType === 'cft' ? 'bg-white text-marine-red shadow-sm' : 'text-red-100 hover:bg-white/10'
            }`}
          >
            CFT
          </button>
      ...
      <div className="space-y-6">
          ...
          <div className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="space-y-6">
                      ...
                      <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-100 dark:border-blue-800/30 border-l-4 border-l-blue-500 dark:border-l-blue-400">
                        <label className="flex items-start gap-3 cursor-pointer">
                          ...
                          <div>
                            <span className="block text-sm font-medium text-blue-900 dark:text-blue-100">
                              High Altitude (&gt;4,500 ft MSL)
                            </span>
              ...
              <div className="space-y-6">
                {/* Score Card */}
                <div
                  className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg border-t-4 border-marine-red"
                  aria-live="polite"
                  aria-atomic="true"
                >
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                  ...
                  <div className="text-center mb-6">
                    ...
                    <div
                      className={`text-lg font-bold px-3 py-1 rounded-full inline-block ${
                        result.scoreClass === '1st Class'
                          ? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300'
                          : result.scoreClass === '2nd Class'
                            ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900 dark:text-yellow-300'
                            : result.scoreClass === '3rd Class'
                  ...
                  {altitude && (
                    <div className="mt-4 p-3 bg-yellow-50 dark:bg-yellow-900/20 text-yellow-800 dark:text-yellow-200 text-xs rounded border border-yellow-200 dark:border-yellow-800 flex items-start gap-2">
                      ...
                      <span>
                        Scores calculated with high altitude consideration (&gt;4,500 ft MSL). Results are for reference
                        only; consult MCO 6100.13A for official adjustments.
                      </span>
                    </div>
                  )}
                ...
                {result.totalScore < 300 && (
                  <ImprovementRecommendations result={result} testType={testType} upperBodyType={upperBodyType} />
                )}
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
              <div className="flex justify-between items-start mb-4">
                <div>
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{visualPlans.pft.description}</p>
                </div>
                ...
                {Array.from({ length: visualPlans.pft.count }).map((_, i) => (
                  <button
                    ...
                    className={`w-2 h-2 rounded-full transition-all ${
                      i === currentCardIndex ? 'bg-marine-red w-4' : 'bg-gray-300 dark:bg-gray-600 hover:bg-gray-400'
                    }`}
                  />
                ))}
              </div>
          ...
          <div className="space-y-6">
            <div className="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mb-4">
                Tap a week to view all cards — tap any cell for a single card
              </p>
                  ...
                  <thead>
                    <tr>
                      ...
                      {cftSchedule.days.map((d) => (
                        <th
                          key={d.day}
                          className="bg-gray-700 text-white p-1.5 text-center font-bold border border-gray-600"
                        >
                          {d.day.toUpperCase()}
                        </th>
                      ))}
            ...
            {cftViewCard !== null && !cftWeekView && (
              ...
              >
                <div
                  className="bg-white dark:bg-gray-800 rounded-xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col"
                  onClick={(e) => e.stopPropagation()}
                >
                  <div className="flex items-center justify-between p-3 border-b border-gray-200 dark:border-gray-700">
                    <span className="font-bold text-sm text-gray-900 dark:text-white">
                      {(() => {
                        const dayInfo = cftSchedule.days.find(
                          (d) => d.offset !== null && getCftCardIndex(cftWeek, d.offset) === cftViewCard
                        );
            ...
            {cftWeekView !== null && (
              ...
              >
                <div
                  className="bg-white dark:bg-gray-800 rounded-xl max-w-3xl w-full max-h-[95vh] overflow-hidden flex flex-col"
                  onClick={(e) => e.stopPropagation()}
                >
                  {/* Header */}
                  <div className="flex items-center justify-between p-3 border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-bold text-gray-900 dark:text-white text-sm">Week {cftWeekView} of 25</span>
                      <button
        ...
        {activeTab === 'pullup' && <PullupProgram />}

        {activeTab === 'plank' && <PlankProgram />}

        {activeTab === 'walk_run' && <WalkToRunProgram />}
      </div>
