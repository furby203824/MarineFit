...
const ExerciseCard = React.memo(({ exercise, index }) => (
  <motion.a
    href={exercise.url}
    target="_blank"
  ...
  >
    <div className="flex items-start justify-between gap-3">
      <div className="flex-1">
        ...
        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{exercise.type}</p>
      </div>
    ...
    <div className="space-y-6">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-gray-200 dark:border-gray-700">
        ...
        <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
          <span className="font-semibold text-marine-red">{exerciseData.metadata.total_exercises}</span> exercises
          available
        </div>
      </header>
      ...
      <div className="text-sm text-gray-500 dark:text-gray-400">
        Showing <span className="font-semibold text-gray-900 dark:text-white">{filteredExercises.length}</span>{' '}
        exercises
      ...
      <div className="space-y-8">
        {Object.entries(groupedExercises).map(([type, exercises]) => {
          ...
          return (
            <motion.div key={type} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
              {/* Category Header */}
              <div className="flex items-center gap-3">
