...
const EventStandardsModal = ({ isOpen, onClose, eventType, gender }) => {
  ...
  const getEventData = () => {
    switch (eventType) {
      ...
      case 'plank':
        return {
          title: 'Plank Standards',
          max: { male: { '21-25': 225 }, female: { '21-25': 225 } },
          min: { male: { '21-25': 40 }, female: { '21-25': 40 } },
          type: 'time',
          unit: 'seconds',
        };
      case 'run':
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <motion.div
        ...
        role="dialog"
        aria-modal="true"
        aria-label="Event Scoring Standards"
      ...
      >
        ...
        <div className="max-h-[50vh] overflow-y-auto">
          {scoringTable.length > 0 ? (
                ...
                {scoringTable.map((row, idx) => (
                  ...
                  >
                    <td className="px-4 py-2 text-gray-800 dark:text-gray-200">
                      <span
                        className={`inline-block w-8 text-center rounded px-1 py-0.5 text-xs font-bold ${row.score >= 90 ? 'bg-green-100 text-green-800' : row.score >= 70 ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800'}`}
                      >
                        {row.score}
                      </span>
                    </td>
            ...
            <div className="p-8 text-center text-gray-500">No scoring data available for this combination.</div>
          )}
        </div>
