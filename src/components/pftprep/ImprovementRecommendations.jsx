  ...
  return hittExercises
    .filter((ex) => {
      const tagMatch =
        ...
        ex.tags &&
        ex.tags.some((tag) =>
          criteria.tags.some((criteriaTag) => tag.toLowerCase().includes(criteriaTag.toLowerCase()))
        );
      const nameMatch =
        criteria.exercises && criteria.exercises.some((name) => ex.name.toLowerCase().includes(name.toLowerCase()));
  ...
  return (
    <div className="flex flex-col">
      ...
      >
        ...
        <div className="text-xs flex items-center gap-1 text-marine-red font-semibold group-hover:underline bg-white dark:bg-gray-600 px-2 py-0.5 rounded border border-gray-200 dark:border-gray-500 shadow-sm">
          {isOpen ? 'Hide' : 'Show'}
    ...
    return (
      <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4 text-center">
        ...
        <p className="text-sm text-green-600 dark:text-green-400 mt-1">
          All events scored 80+. Keep up the excellent work!
        </p>
      </div>
    );
  }
              ...
              <CollapsibleSection title="Recommended HITT Exercises">
                <div className="space-y-2">
                  {matchedExercises.map((ex) => (
                    <div
                      key={ex.id}
                      className="flex items-center justify-between bg-white dark:bg-gray-600 p-2 rounded border border-gray-100 dark:border-gray-500"
                    >
                      <div className="flex items-center gap-2">
