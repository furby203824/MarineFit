...
const PTCoach = () => {
  ...
  const confirmSwapExercise = (newExercise) => {
    ...
    // Preserve the prescription from the original exercise
  ...
  // Filtered exercises for swap picker (same category, not already in workout)
  const swapFilteredExercises = useMemo(() => {
    ...
    return hittExercises.filter(ex => {
      ...
      // Can't already be in the workout (except current one being swapped)
      ...
      // Search filter
      if (swapSearchQuery && !ex.name.toLowerCase().includes(swapSearchQuery.toLowerCase())) {
        return false;
      }
      return true;
    });
      ...
      <AnimatePresence>
        {showSwapPicker && swapTarget && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4"
            onClick={() => { setShowSwapPicker(false); setSwapTarget(null); }}
          >
            <motion.div
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
            ...
            >
              ...
              {/* Exercise List */}
              <div className="flex-1 overflow-y-auto p-4">
                {swapFilteredExercises.length === 0 ? (
                  <div className="text-center py-8 text-gray-500">
                  ...
                  <div className="grid gap-2">
                    {swapFilteredExercises.map((ex) => (
                      <button
                        key={ex.id}
                        onClick={() => confirmSwapExercise(ex)}
