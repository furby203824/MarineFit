...
const MAX_SCORE = 100;
const MIN_SCORE = 40;
const SCORE_RANGE = MAX_SCORE - MIN_SCORE; // 60

// Classification thresholds
const FIRST_CLASS_THRESHOLD = 235;
const SECOND_CLASS_THRESHOLD = 200;
const THIRD_CLASS_THRESHOLD = 150;

// Plank is not age-dependent per USMC standards
const PLANK_MAX_SECONDS = 225; // 3:45 for 100 points
const PLANK_MIN_SECONDS = 40; // 0:40 minimum for any points
...
const interpolateScore = (value, minVal, maxVal, isTimeBased = false) => {
  if (isTimeBased) {
    ...
    if (value <= maxVal) return MAX_SCORE;
    if (value >= minVal) return MIN_SCORE;
    // Linear interpolation: as time increases, score decreases
    const ratio = (value - maxVal) / (minVal - maxVal);
    return Math.round(MAX_SCORE - ratio * SCORE_RANGE);
  } else {
    ...
    return Math.round(MIN_SCORE + ratio * SCORE_RANGE);
  }
};
...
export const calculateCFTScore = (gender, ageGroup, inputs) => {
  ...
  return {
    ...
    totalScore,
    classification,
    isAltitude, // Pass through for UI warning
  };
};

// Helper Functions
export const getClassification = (score) => {
  if (score >= FIRST_CLASS_THRESHOLD) return { class: 'first-class', name: '1st Class' };
  if (score >= SECOND_CLASS_THRESHOLD) return { class: 'second-class', name: '2nd Class' };
  if (score >= THIRD_CLASS_THRESHOLD) return { class: 'third-class', name: '3rd Class' };
...
export const calculatePlankScore = (minutes, seconds) => {
  ...
  if (totalSeconds < PLANK_MIN_SECONDS) return 0;
  if (totalSeconds >= PLANK_MAX_SECONDS) return MAX_SCORE;

  // Linear interpolation between MIN_SCORE at 40s and MAX_SCORE at 3:45
  const ratio = (totalSeconds - PLANK_MIN_SECONDS) / (PLANK_MAX_SECONDS - PLANK_MIN_SECONDS);
  return Math.round(MIN_SCORE + ratio * SCORE_RANGE);
};
...
export const getRequiredUpperBodyReps = (gender, ageGroup, type, targetScore) => {
  ...
  if (type === 'pullups') {
    ...
    if (targetScore >= MAX_SCORE) return maxReps;
    if (targetScore <= MIN_SCORE) return minReps;

    // Reverse interpolation
    const ratio = (targetScore - MIN_SCORE) / SCORE_RANGE;
...
export const getRequiredPlankTime = (targetScore) => {
  ...
  if (targetScore >= MAX_SCORE) return '3:45';
  if (targetScore <= MIN_SCORE) return '0:40';

  const ratio = (targetScore - MIN_SCORE) / SCORE_RANGE;
  const totalSeconds = Math.ceil(PLANK_MIN_SECONDS + ratio * (PLANK_MAX_SECONDS - PLANK_MIN_SECONDS));
